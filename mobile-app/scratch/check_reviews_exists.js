...
async function checkReviewsSchema() {
  const { data, error } = await supabase.from('reviews').select('*').limit(0);
  if (error) {
    console.log('Error:', error.message);
  } else {
    // This doesn't give columns if empty. 
    // I'll try to insert a dummy review and rollback or just look for a product with reviews.
